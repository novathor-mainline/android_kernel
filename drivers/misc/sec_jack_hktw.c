...
#define up_adc_low 101
#define down_adc_high 450
...
struct sec_jack_info {
	...
	struct timespec tp_button_before;
	struct timespec tp_button_now;
	bool before_key;
	int det_r_irq;
	int det_f_irq;
	int buttons_r_irq;
...
static void sec_jack_buttons_work(struct work_struct *work)
{
	...
	hi->tp_button_now=current_kernel_time();
	 distance_time=(hi->tp_button_now.tv_nsec-hi->tp_button_before.tv_nsec)/1000000;
	if(distance_time<500&&hi->before_key==true&&adc==0)
       		 adc=1500;
	...
	for (i = 0; i < pdata->num_buttons_zones; i++)
		...
		if(adc>=up_adc_low&&adc<=down_adc_high)
		{
			hi->before_key=true;
			hi->tp_button_before=current_kernel_time();
		}
		else
		{	
			hi->before_key=false;
